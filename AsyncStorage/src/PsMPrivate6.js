...
import Fire from "../Screen.js/Fire";
...
export default function PrivateScreen(props) {
    ...
    const save = async () => {
      try {
        await AsyncStorage.setItem("MyNames4", name);
      } catch (err) {
        alert(err);
      } 
    };
    ...
    const load = async () => {
      try {
        let name = await AsyncStorage.getItem("MyNames4");
    ...
    const remove = async () => {
      try {
        await AsyncStorage.removeItem("MyNames4")
      } catch (err) {
        alert(err)
      } finally {
         setName("")
